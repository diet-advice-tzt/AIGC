...
const Track: React.FC = () => {
  ...
  const connectWs = useCallback(() => {
    ...
    ws.onmessage = (evt) => {
      try {
        ...
        // 后端返回 UserTrackVO: { trackId, status, recordTime, remark }
  ...
  const isTracking = geoWatchRef.current !== null
  ...
  return (
    <div style={{ display: 'flex', gap: 20, height: 'calc(100vh - 56px - 56px)', minHeight: 500 }}>
      ...
      <div style={{
        ...
        {/* Map */}
        <div ref={mapRef} style={{ flex: 1, background: 'var(--bg)' }}>
          {/* 高德 AMap 加载后会渲染在这里，否则显示占位 */}
          {!(window as any).AMap && (
            <div style={{
